...
public class CohortValidator {
    ...
    public static void validateAddTeam(Cohort cohort, Team team) {
    ...
    public static void validateRemoveTeam(Cohort cohort, Team team) {
    ...
    public static void validateMarkAsCompleted(Cohort cohort) {
    ...
    public static void validateArchive(Cohort cohort, @NonNls String reason) {
    ...
    public static void validateUpdateDetails(Cohort cohort, ChronoLocalDate startTerm, ChronoLocalDate endTerm) {
    ...
    public static void validateUpdateMaxStudents(Cohort cohort, int maxStudents) {
        ...
        if (maxStudents < MIN_STUDENTS || maxStudents > MAX_STUDENTS) {
            throw new ValidationException(
                String.format("Max students must be between %d and %d",
                    Integer.valueOf(MIN_STUDENTS), Integer.valueOf(MAX_STUDENTS)));
        }
    }

    private static void validateBasicFields(Cohort cohort, ValidationError error) {
    ...
    private static void validateDates(Cohort cohort, ValidationError error) {
        ...
        if (cohort.getStartTerm().isBefore(LocalDate.now().minusYears(1L))) {
            throw new ValidationException(error.code, "Start term cannot be more than 1 year in the past");
        }
    }

    private static void validateCapacity(Cohort cohort, ValidationError error) {
        int maxStudents = cohort.getAssignment().maxStudents();
        if (maxStudents < MIN_STUDENTS || maxStudents > MAX_STUDENTS) {
            throw new ValidationException(error.code,
                String.format("Student capacity must be between %d and %d", Integer.valueOf(MIN_STUDENTS), Integer.valueOf(MAX_STUDENTS)));
        }
    }
    ...
    private static void validateYearCapacity(Cohort cohort, School school) {
        long cohortsInYear = school.getCohorts().stream()
            .filter(c -> c.getAssignment().year().equals(cohort.getAssignment().year()))
            .count();

        if ((long) MAX_COHORTS_PER_YEAR <= cohortsInYear) {
            throw new ValidationException(ValidationError.YEAR_CAPACITY_EXCEEDED.code,
                String.format("Maximum cohorts per year (%d) exceeded", Integer.valueOf(CohortValidator.MAX_COHORTS_PER_YEAR)));
        }
    }

    private static void validateUniqueName(final Cohort cohort, final School school) {
    ...
    private void validateGradeLevel(final Cohort cohort) {
        ...
        if (!CohortValidator.isValidGradeLevel(assignment.level())) {
            throw new ValidationException("Invalid grade level assigned");
        }
    }

    private static void validateMaxStudents(final Cohort cohort) {
        if (MAX_STUDENTS < cohort.getAssignment().maxStudents()) {
            throw new ValidationException(
                String.format("Cohort exceeds maximum student capacity of %d", Integer.valueOf(CohortValidator.MAX_STUDENTS)));
        }
    }

    private static void validateYearScheduling(final Cohort cohort, final School school) {
    ...
    private static void validateStateTransition(final Cohort cohort) {
    ...
    private static boolean isValidGradeLevel(final GradeLevel level) {
