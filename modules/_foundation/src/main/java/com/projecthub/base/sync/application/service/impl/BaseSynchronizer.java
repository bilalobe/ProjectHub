...
public abstract class BaseSynchronizer<T extends BaseEntity> implements EntitySynchronizer<T> {
    ...
    @Override
    @Transactional
    @Retryable(maxAttempts = BaseSynchronizer.MAX_RETRY_ATTEMPTS, backoff = @Backoff(delay = 1000L), retryFor = SynchronizationException.class)
    ...
    protected List<T> merge(final List<T> local, final List<T> remote) {
        ...
        local.forEach(item -> {
            ...
            if (BaseSynchronizer.shouldUseLocalVersion(localModified, remoteModified)) {
    ...
    private static boolean shouldUseLocalVersion(final ChronoLocalDateTime<LocalDate> localModified, final ChronoLocalDateTime<LocalDate> remoteModified) {
