...
public class H2LocalDataService implements LocalDataService {
    ...
    public <T> List<T> getLocalData(final Class<T> entityClass) {
        final String tableName = entityClass.getSimpleName().toLowerCase(Locale.ROOT);
    ...
    public <T> void saveLocalData(final List<T> entities) {
        ...
        final String sql = H2LocalDataService.generateInsertSql(tableName, entityClass);
    ...
    private static String generateInsertSql(final String tableName, final Class<?> entityClass) {
    ...
    private static class EntityBatchPreparedStatementSetter<T> implements BatchPreparedStatementSetter {
        ...
        public void setValues(@NonNull final PreparedStatement ps, final int i) throws SQLException {
            ...
            for (final Field field : this.fields) {
                ...
                if (null != value) {
                    ...
                    ps.setNull(paramIndex, EntityBatchPreparedStatementSetter.getSqlType(field.getType()));
                    paramIndex++;
                }
            }
        }
        ...
        private static int getSqlType(final Class<?> type) {
