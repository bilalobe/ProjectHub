...
public class TokenManagementService {
    ...
    public TokenPair generateTokenPair(final AppUser user) {
        ...
        return new TokenPair(
            accessToken,
            refreshToken,
            Long.valueOf((long) this.properties.accessTokenValidityMinutes() * 60L)
        );
    }
    ...
    private void enforceMaximumTokenLimit(final String username) {
        ...
        if (activeTokenCount >= (long) this.properties.maxActiveTokens()) {
    ...
    public static String generateAccessToken(final AppUser user) {
        return null;
    }
}
