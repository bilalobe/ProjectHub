...
public class SyncCohortEventAdapter implements CohortEventAdapter {
    ...
    public void publish(final CohortDomainEvent event) {
        ...
        try {
            SyncCohortEventAdapter.log.debug("Publishing sync event of type {}: {}", SyncCohortEventAdapter.getEventType(event), event);
    ...
    private static String getEventType(final CohortDomainEvent event) {
        return switch (event) {
