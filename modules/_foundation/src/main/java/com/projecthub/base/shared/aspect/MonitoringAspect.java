...
public class MonitoringAspect {
    ...
    public Object monitorPerformance(final ProceedingJoinPoint joinPoint) throws Throwable, Exception {
        ...
        try {
            ...
            MonitoringAspect.logMethodExecution(methodName, executionTime, null);
            return MonitoringAspect.filterSensitiveData(result);
        } catch (final Exception e) {
            final long executionTime = System.currentTimeMillis() - startTime;
            MonitoringAspect.logMethodExecution(methodName, executionTime, e);
            throw e;
        }
    }

    private static void logMethodExecution(final String methodName, final long executionTime, final Exception error) {
        final String logMessage = String.format("Method: %s | Duration: %dms | Status: %s",
            methodName, Long.valueOf(executionTime), null == error ? "SUCCESS" : "ERROR");
        if (null == error) {
    ...
    private static Object filterSensitiveData(final Object result) {
