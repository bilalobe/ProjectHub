...
public class UpdateServiceImpl implements UpdateService {
    ...
    public <T extends BaseEntity> void updateBothStores(final List<T> mergedData, final Class<T> entityClass) {
        try {
            UpdateServiceImpl.logger.debug("Updating local store with {} entities of type {}", Integer.valueOf(mergedData.size()), entityClass.getSimpleName());
            this.localDataService.clearLocalData(entityClass);
            this.localDataService.saveLocalData(mergedData);

            UpdateServiceImpl.logger.debug("Updating remote store with {} entities", Integer.valueOf(mergedData.size()));
